...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Provision the separate Jason Datto RMM execution OpenBao secret, "
...
def main() -> int:
    ...
    if arguments.credential_dir.exists():
        raise ProvisioningError(
            "The Datto RMM execution credential directory already exists. "
    ...
    try:
        ...
        raise ProvisioningError("The Datto RMM execution policy file is unavailable.") from error
    ...
    try:
        if request_status(
            ...
            raise ProvisioningError(
                "The Datto RMM execution policy already exists; refusing to overwrite it."
            )
        if request_status(
            base_url=arguments.base_url,
            ...
            raise ProvisioningError(
                "The Datto RMM execution AppRole already exists; refusing to overwrite it."
            )
        if request_status(
            base_url=arguments.base_url,
            path=execution_path,
            ...
            raise ProvisioningError(
                "The Datto RMM execution secret already exists; refusing to overwrite it."
            )
        ...
        if not api_key or not api_secret:
            raise ProvisioningError("Datto RMM execution API credentials may not be empty.")
        ...
        if execution_values.get("api_url") != api_url:
            raise ProvisioningError("Datto RMM execution API URL verification failed.")
        if execution_values.get("api_key") != api_key:
            raise ProvisioningError("Datto RMM execution API key verification failed.")
        if execution_values.get("api_secret") != api_secret:
            raise ProvisioningError("Datto RMM execution API secret verification failed.")
        execution_version = kv2_version(execution_record, "Datto RMM execution")
        if execution_version != 1:
            raise ProvisioningError(
                "Datto RMM execution secret was not created as a new isolated record."
            )
        ...
        api_key = ""
        api_secret = ""
        print("Datto RMM execution secret created as a separate record.")
        print("Datto RMM execution least-privilege OpenBao policy installed.")
        print("Datto RMM execution AppRole configured and isolation verified.")
