...
**Updated:** 2026-09-24
**Status:** Production runtime and MCP are healthy on independently pinned deployed revisions. Runtime image `sha256:94b3c5bb3c07863f5b1836c62be91d020423be1eafd2347002f433478047c362` is built from source `fe75d15304a294fe20193c5c837f9f99be74e4ee`; MCP image `sha256:785279fe3ea25145dea7e55bd4256656999adc4bd7fc2f838cbf66877f7ba901` is built from source `5244e9e41ac86a366fc475b37be0559919fdc968` and includes the production-proven SUPPORT-CAP-019 direct ticket-update canonicalization fix. These images back the canonical `jason-runtime:production`, `jason-runtime:local`, and `jason-mcp:production` aliases. The SUPPORT-CAP-019 MCP promotion preserved the immediately previous live MCP container as stopped rollback container `jason-mcp-pilot-rollback-20260924T151156Z`; verify current rollback tags and container state before any rollback operation rather than relying on older release labels. Runtime and MCP remain hardened; Central Orchestrator remains authoritative; `direct_provider_access=false`; and the governed write-capability list is unchanged. Backup.net/UniView is production-selected on `JASON_BACKUP_NET_ACCESS_PROFILE=full_access` using the isolated `backup_net.fullaccess` KV/AppRole identity mounted read-only into both runtime and MCP. The existing validated company-to-customer boundary and `provider-read:backup_net` / `observe` authority still govern the four provider reads. Full-access production acceptance against DGV-50859 passed on 2026-09-24 across asset search/read, backup search, and BackupIQ alert search; durable evidence is `docs/sessions/BackupNet-Full-Access-Production-Activation-2026-09-24.md`. The prior read-only credential identity remains separately preserved for rollback. The published Backup.net Public API currently advertises GET operations only, so full-access provider credentials do not create a Jason write/remediation capability; no Backup.net mutation capability is registered.
...
- **Next safe actions:** `main` remains authoritative and branch-protected. Backup.net full-access credential selection is production-accepted for the existing governed read surface. The broader BackupIQ remediation/write work remains a separate workstream and requires provider-documented mutation operations before any Jason write capability can be registered. Current unrelated feature/support branches must continue through normal protected review; do not infer mutation authority from provider credential scope.
