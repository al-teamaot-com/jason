...
**Updated:** 2026-09-24
**Status:** Production runtime and MCP are healthy on deployed source `8362eaa4e236d585711d51b775f4f551bab78b6f`. Runtime image `sha256:9e088c99eadc51d79e0166a9e4f42282b5a4b308fbc651ef681a6a2172aa3b61` and MCP image `sha256:2602eceea6eac75c73d9133e72a8d84c1f3b7621d4c5829b0e907aa85dd91650` back the canonical `jason-runtime:production`, `jason-runtime:local`, and `jason-mcp:production` aliases. The immediate previous deployed release `699bf0a836888ca0565369ba7a8db8f0c115caf0` is retained as `jason-runtime:rollback-current` and `jason-mcp:rollback-current`, with one stopped runtime/MCP rollback container pair preserved. Runtime and MCP remain hardened; Central Orchestrator remains authoritative; `direct_provider_access=false`; and the existing governed write surface remains unchanged. The read-only Backup.net/UniView Endpoint Backup foundation is production-enabled on both runtime and MCP, uses the dedicated OpenBao AppRole mounted read-only, requires the validated company-to-customer boundary, and is authorized only by `provider-read:backup_net` / `observe`. Controlled DGV-50859 acceptance passed on 2026-09-24; durable evidence is `docs/sessions/BackupNet-Production-Activation-2026-09-24.md`. Source now also defines an isolated `backup_net.fullaccess` credential/AppRole profile and explicit runtime selection for a future provider full-access credential; this profile is not production-enabled. The published Backup.net Public API currently advertises GET operations only, so no Backup.net write/remediation capability is registered from credential permission alone.
...
- **Next safe actions:** `main` remains authoritative and branch-protected. Backup.net issue #216 / PR #218 is production-accepted for governed read-only Endpoint Backup and BackupIQ diagnostics. Current open feature PRs are #222 REFLECT-001, #226 global ticket-check notes, and #227 patch-reliability intelligence. The broader BackupIQ remediation playbook remains a separate workstream; do not infer write/remediation authority from the read-only Backup.net activation.
