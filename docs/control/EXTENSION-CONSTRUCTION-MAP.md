...
| Internal service / runtime component | `docs/architecture/J-100-Reference-Architecture.md`, relevant JKD/INF record, and deployment architecture | `jason-runtime`, OpenBao, OpenClaw deployment/runbook patterns | Defined responsibility and dependency boundary; service identity; network/secret mounts by reference; health verification; hardened runtime controls; rollback; System Registry declared/observed/verified state |
...
| Deployment / operational procedure | `docs/operations/README.md` and relevant INF/runbook | Existing runtime/OpenBao/provider deployment runbooks | Prerequisites; authority; observation vs mutation; stop conditions; rollback; verification; evidence retention; no secret printing; no silent drift repair |
