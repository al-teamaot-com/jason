...
## Native-platform-first monitoring aggregation
Per Owner direction, posture collection must prefer capabilities and operational processes Jason already has rather than creating parallel collectors or replacement monitoring logic. For DRMM monitoring posture, the production proof therefore uses the existing governed `endpoint.alert.search` capability against each exact endpoint UID returned by the already-proven complete site inventory. No new alert collector was introduced.

On 2026-09-19, exact endpoint-scoped open-alert reads were completed across all 25 Atomic Windows endpoints. Every returned alert independently identified Atomic's bound DRMM site UID. The sweep found 13 open alerts across 8 endpoints; 17 endpoints returned zero open alerts. Examples include the Critical Datto AV threat plus HOSTS/Security-log monitoring on `Atomic-50291`, monitoring failures on `APD-51715`, `APD-50759`, and `APD-50399`, a `NOT INSTALLED` monitor result on `APD-HYPERV`, Group Policy connectivity on `Server-2016`, Local Administrators drift on `APD-51677`, and local-user drift on `APD-50214`.

This establishes a client-wide `confirmed_gap` for `ENDPOINT-MONITORING`: complete endpoint coverage exists and authoritative open actionable alerts are present. It does **not** mean every alert is an unresolved client problem; individual alerts still require their existing playbook/triage process, ticket correlation, exception handling, and normal closeout rules. Informational alerts remain evidence but should not independently create a client posture gap unless AOT policy says they require action.

The earlier account-level `management.alert.search` response that leaked cross-site results remains excluded. Existing exact endpoint-scoped reads are sufficient, so no replacement alert API or custom collection subsystem is needed.
