...
Continuation-based and recovery-retry approval paths are not exempt from execution-plan binding. Their durable continuation/recovery guards provide one-time consumption, while the Central Orchestrator independently prepares the concrete provider plan twice and requires exact fingerprint equality before invocation even when the flow does not use the MCP governed-execution approval ledger. The provider maximum execution time is enforced as one shared absolute deadline across both preparations and invocation.

The repeatable operator procedure for ambiguous provider outcomes is `docs/operations/Governed-Mutation-Indeterminate-Recovery.md`.
