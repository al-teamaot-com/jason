...
When an exact provider mutation contains a secret-bearing field that is itself authority-relevant, Jason binds the value through a deterministic one-way commitment rather than persisting the secret. The adapter stores the exact secret only in ephemeral provider-private prepared state, places a cryptographic digest (currently SHA-256 where implemented) in the authorized plan, and re-computes/requires the same digest immediately before invocation. This exception is for secret-bearing material only; ordinary mutation fields remain bound by their exact normalized provider values.

Provider-private opaque prepared state must not be included in normal object representations, audit payloads, fingerprints, or persistence. Shared prepared-plan containers suppress opaque state from `repr()` as defense in depth; adapters remain responsible for not logging their own secret-bearing private state.
...
The execution-plan binding described above began at commit `56b0e91fe376fb270ac521c5c1754bfa12aafdb5`. Follow-on remediation on `fix/security-remediation-20260923` hardened deterministic secret-safe plan serialization and Autotask method/path/target revalidation at `0ed6911`, then adapted the remaining identified approval-governed action surface at `e41e572`: Autotask procurement, Datto RMM component execution, Datto RMM site-variable create/update, Datto EDR scan execution, Datto alert resolution, and Microsoft Teams proactive message send. Site-variable secret values are authority-bound through SHA-256 commitments without entering persisted plan/audit material. Shared opaque prepared state is excluded from normal `repr()`. The combined focused regression set is **141/141 PASS**. The control is still **not deployed or production-accepted**; the last verified production security boundary remains the approval-replay deployment revision `5f89f3af82081e75e97d66e523222e5564648163`. Production deployment, clean-build/rollback proof, and bounded XYZ live acceptance remain pending and must not be represented as completed.
