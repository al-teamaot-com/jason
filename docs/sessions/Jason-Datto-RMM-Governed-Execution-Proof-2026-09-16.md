...
Prove the governed Datto RMM execution path end-to-end without weakening governance: confirm reads, execute one safe bounded diagnostic component against the controlled endpoint only after explicit per-execution approval, capture a durable provider job reference, and verify terminal completion through the governed read path.
