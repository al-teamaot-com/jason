...
- **Production checkpoint (2026-09-20):** `service.entity.fields.describe` is live in production image `jason-mcp:customer-comm-fields-5f33724`, source `5f33724cb980fc3513a0f71f8c06232af335a197`, with `direct_provider_access=false`. Governed read of live `TicketNotes` field metadata succeeded (`corr_mcp_76a48c5ae095479f9d7cd76439f661a4`). AOT currently exposes `publish` values only for `All Autotask Users`, `Internal Project Team`, and `Internal & Co-Managed`; there is no customer/public publication value. The live `noteType` list also has no dedicated Jason communication type.
- **Implementation checkpoint (2026-09-20, continued):** Added a fail-closed Jason customer-communication trigger contract. It requires an exact ticket with a positive `contactID`, dynamically resolves the active `Jason Customer Communication` note type from live TicketNotes picklist metadata, dynamically resolves `All Autotask Users` publication, bounds title/message size, and refuses to build a provider payload if the dedicated note type is absent or ambiguous.
- **Required Autotask-native setup:** Create one Note Type named exactly `Jason Customer Communication` under Application-Wide (Shared) Features > Note Types. Create one Service Desk workflow rule whose event is `Note Added By = Autotask Resource`, condition limits the event to that exact Note Type, recipient is `Ticket Contact`, sender is the approved AOT support address, and notification uses the approved Jason/customer-update Notification Template. Do not trigger on all resource notes.
- **Remaining acceptance:** After the note type/workflow exist, wire the trigger contract into a distinct high-risk governed action; pre-read exact ticket and contact; preview the message; create exactly one trigger note; read back the exact note; then verify one matching NotificationHistory record for the same ticket/contact/template. Resolve `SUPPORT-CAP-016` first if NotificationHistory remains permission-blocked. Preserve `direct_provider_access=false` and keep the existing internal-note action separate.
- **Decision owner:** Jason Governance Authority / AOT Owner
