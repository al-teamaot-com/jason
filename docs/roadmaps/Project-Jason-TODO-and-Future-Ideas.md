...
- **2026-09-25 production acceptance:** Hardened revision `cb87d278fb9d9b828672e4158dad01c5d43d0ead` deployed to runtime and MCP with `itglue-autotask-entra-procurement-mail-contract-catalog-v7`; deployment hardening, health checks, and MCP governance post-check all passed. Normal MCP `service.contract.search` against Atomic Plumbing / Autotask company `333` returned three active contracts. Exact `service.contract.read` for contract `29683369` with `company_id=333` returned exactly the Atomic Essentials MSA. Repeating that exact contract ID under `company_id=311` returned zero items, confirming no cross-client contract disclosure. Contract reads remain read-only and client-isolated. The current Autotask identity still reports `userAccessForUpdate=None`, so no contract mutation capability is enabled or planned until a dedicated write permission and governed mutation design are separately approved and accepted.
