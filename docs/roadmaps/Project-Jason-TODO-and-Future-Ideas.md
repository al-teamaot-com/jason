...
- **Priority:** P1
- **Status:** Read-only production foundation complete; full-access credential profile production-active; provider-supported write operations pending
- **Risk level:** High
...
- **Full-access credential checkpoint (2026-09-24):** Production is active on isolated logical secret `backup_net.fullaccess`, dedicated KV/AppRole/runtime artifacts, and `JASON_BACKUP_NET_ACCESS_PROFILE=full_access`. The existing read-only credential remains separately preserved for rollback. Provider credential scope is not Jason authority.
- **Current provider limitation:** The published Backup.net Public API OpenAPI contract advertises GET operations only as of 2026-09-24. Endpoint Backup v2 product documentation confirms operational UI actions such as on-demand backup, pause/resume, policy/retention changes, recovery operations, restore cancellation, BMR, and asset deletion, but no provider-supported third-party API mutation contract for those v2 actions has been identified. A full-access credential therefore does not create a documented mutation endpoint. No private/UI endpoint may be substituted and no Backup.net write capability may be registered from credential permission alone.
...
- **Current API reference:** `docs/operations/BackupNet-Endpoint-Backup-API-Current-State-2026-09-24.md`.
...
- **Remaining acceptance:** Normalize complete DRMM inventory evidence across the bound client; obtain/authorize IT Glue organization evidence without bypassing its information-release gate; consume the now-live client-bounded Backup.net Endpoint Backup reads for backup posture instead of preserving Endpoint Backup as unavailable; preserve unavailable status only for controls whose provider evidence is still not integrated; add report/Grafana evidence after live proof.
- **Decision owner:** Jason Governance Authority / AOT Owner
