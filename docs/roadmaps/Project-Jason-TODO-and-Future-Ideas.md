...
- **Status:** In progress — source adapter compatibility complete; shared-framework review, production deployment, and live acceptance pending
- **Risk level:** Critical
...
- **Confirmed evidence:** Approval replay originally created duplicate Autotask notes `30506555` and `30506556`. After the replay fix, controlled production replay created only note `30506631`; the replay was `deduplicated` and did not invoke the provider again. Core execution-plan binding is `56b0e91fe376fb270ac521c5c1754bfa12aafdb5`; follow-on remediation on `fix/security-remediation-20260923` is checkpointed at `0ed6911` and `e41e572`. The currently identified approval-governed action surface is adapted at source level, including Autotask ticket/create/note/procurement, Datto RMM component execution/site variables/alert resolution, Datto EDR scan execution, and Teams proactive send. Combined focused regression: 141/141 PASS. Chronological record: `docs/sessions/2026-09-23.md`.
...
- **High-priority work, in order:**
  1. **Finish shared-framework review.** Preserve the now-complete source adapter compatibility; complete remaining cross-cutting review such as total execution deadline budgeting and recovery/indeterminate-outcome behavior without introducing a legacy bypass.
  2. **Add/retain execution-plan verification in CI/security regression testing.** Required cases include changed provider, target, normalized payload, symbolic mapping, operation/path, secret commitment, exact successful retry/deduplication, secrets/volatile hostnames excluded, and zero provider writes on every denied case.
  3. **Clean deployment/rebuild verification.** Resolve the Docker overlay-chain issue; prove a clean build from authoritative remediation Git reproduces replay/idempotency and execution-plan behavior; preserve and verify the current production rollback.
  4. **Deploy the verified remediation revision to production.** Verify the exact live revision/image after deployment with no provider mutation.
  5. **Complete bounded XYZ live acceptance.** Only after successful deployment, use `XYZ Test Company` ticket `T20211001.0014` / internal ticket ID `29860`; perform one harmless reversible `service.ticket.update`; verify intent fingerprint, execution-plan fingerprint, selected provider, exact target, normalized payload, symbolic-resolution evidence, prepare/re-prepare fingerprint stability, exactly one provider write, no execution-plan mismatch, and post-write provider state/readback matching the authorized execution plan.
