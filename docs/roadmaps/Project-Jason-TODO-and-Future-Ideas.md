...
- **Status:** In progress — source remediation, clean candidate builds, and rollback verification complete; production deployment and live acceptance pending
- **Risk level:** Critical
...
- **High-priority work, in order:**
  ...
  3. **Deploy the verified remediation revision to production.** This remains a separate approval boundary. Deploy the verified candidate images, preserving the pinned rollback assets, then verify exact live revision/image, health, tool/governance surface, Central Orchestrator, and `direct_provider_access=false` with no provider mutation.
