...
- **Priority:** P1
- **Status:** Shadow-ready — two-tier authority, OpenBao, boundaries, and v6 shadow validated; blocked only on Microsoft RBAC cache propagation and final live acceptance
- **Risk level:** High
...
- **Live acceptance checkpoint (2026-09-21):** Both OpenBao mail identities are provisioned at KV version 1 and both Microsoft client boundaries are `validated`. Tenant-wide metadata search succeeded against `al@teamaot.com` with no body exposure. Empty-scope content access returned HTTP 403 as required. After adding `al@teamaot.com` as the sole direct opt-in member, Exchange `Test-ServicePrincipalAuthorization` reports `Application Mail.Read` with `InScope=True`; Graph still returns 403 during Microsoft RBAC cache propagation. Shadow image `jason-mcp:communications-evidence-e4d8b4f` runs v6 with no host port and exactly one approved full-content mailbox; production remains on the prior profile with no active mail capabilities.
