...
- **Priority:** P1
- **Status:** In progress — first verified production case captured 2026-09-24; later same-client recurrence reuse proof pending
- **Risk level:** Moderate
...
- **First verified production case (2026-09-24):** `resmem-20260924-hitt-ws06-sysmon-diskspace` was ingested for Hitt Electric / Autotask company `196` from exact ticket `T20260923.0003` / ID `140932`. Ticket evidence proves the root cause: `C:\Sysmon` consumed approximately 426.38 GiB of a 464.66 GiB C: volume after repeated generic cleanup had produced only marginal gains. Owner-approved bulk Sysmon archive removal reclaimed approximately 311.22 GiB. Terminal governed verification showed 355,466,973,184 bytes free (~71%), no open DRMM alerts, AV `RunningAndUpToDate`, fully patched, and `reboot_required=False`; the ticket was completed at `2026-09-24T15:31:08.860Z`. The stored case is `verified` and `grants_authority=false`. Governed organization aggregate summary now reports `case_count=1`, `raw_cases_exposed=false`, correlation `corr_mcp_a2122d0430a6409882728d7e769c4f84`. Direct case read/search without a current client context correctly denied with `client_context_required`, including when a caller supplied `client_id=196`; this is accepted client-isolation evidence. The remaining TODO-OPS-001 completion proof is a later materially similar Hitt incident where the normal investigation workflow establishes current client context, retrieves this historical case, demonstrates evidence-only diagnostic prioritization, and preserves all normal approval/disruption controls.
