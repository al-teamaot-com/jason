...
- **Status:** In progress — source adapter and shared-framework remediation complete; clean deployment, rollback verification, and live acceptance pending
- **Risk level:** Critical
...
- **High-priority work, in order:**
  ...
  2. **Clean deployment/rebuild verification.** Resolve the Docker overlay-chain issue; prove a clean build from authoritative remediation Git (source checkpoint `bf072af` or later documentation-only descendant) reproduces replay/idempotency and execution-plan behavior; preserve and verify the current production rollback.
  3. **Deploy the verified remediation revision to production.** Verify the exact live revision/image after deployment with no provider mutation.
  4. **Complete bounded XYZ live acceptance.** Only after successful deployment, use `XYZ Test Company` ticket `T20211001.0014` / internal ticket ID `29860`; perform one harmless reversible `service.ticket.update`; verify intent fingerprint, execution-plan fingerprint, selected provider, exact target, normalized payload, symbolic-resolution evidence, prepare/re-prepare fingerprint stability, exactly one provider write, no execution-plan mismatch, and post-write provider state/readback matching the authorized execution plan.
