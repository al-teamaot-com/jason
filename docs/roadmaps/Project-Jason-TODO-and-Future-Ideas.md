...
- **Priority:** P1
- **Status:** In progress — approved-template catalog and autonomous template-gap proposal workflow implemented; governed NotificationHistory read remains blocked by `SUPPORT-CAP-016`; direct named-template dispatch is not exposed by the documented Autotask API
- **Risk level:** High
...
- **Communication-template engineering checkpoint (2026-09-19):** Jason now has an authoritative approved-template catalog model plus durable de-duplicated Communication Template Requests. If no approved purpose/audience match exists, `PROMPT-COMM-001` drafts reusable content and the deterministic AOT renderer produces the proposed HTML body using the established AOT branding shell. Proposed templates remain unapproved until reviewed/published and cannot grant send authority. Grafana/Prometheus artifacts are implemented without exporting HTML content.
- **Remaining acceptance:** Restore minimum supported NotificationHistory read permission; populate the approved catalog with AOT-reviewed template content and map known Autotask template names; production-deploy the request/catalog runtime and dashboard; prove one real no-fit request and review/publish cycle; implement preview + audience validation; dispatch only through a supported governed provider surface; verify NotificationHistory or equivalent post-send evidence; preserve normal approval rules and `direct_provider_access=false`.
- **Decision owner:** Jason Governance Authority / AOT Owner
...
- **Production checkpoint (2026-09-19, complete DRMM inventory):** Site-only device discovery was fixed to enumerate the authorized Datto account to completion before exact-site filtering. Atomic production proof examined four provider pages / 749 account resources and returned 29 exact-site resources with `discovery_complete=true`; all 29 exact resource reads independently confirmed the Atomic site UID. Twenty-five are Windows endpoints and four are network devices. Complete Windows endpoint evidence establishes a managed-AV gap (`APD-50213` Defender `NotRunning`; `APD-HYPERV` Datto AV `RunningAndNotUpToDate`; `APD-50712` Defender rather than Datto AV requires exception/policy evidence).
- **Technology lifecycle checkpoint (2026-09-19):** Added deterministic AOT Windows lifecycle baseline with vendor-support dates, edition/release awareness, explicit ESU and approved-exception states, and fail-closed LTSC/LTSB handling. Unsupported status is evidence for a posture proposal only and grants no remediation authority.
- **OS/EDR checkpoint (2026-09-19):** Applied the lifecycle baseline to Atomic: client OS posture is a confirmed gap due to base-support-ended Windows 10 22H2 and Hyper-V Server 2012 endpoints absent ESU/exception evidence; Windows 11 build 22621 remains unknown rather than guessed. Exact EDR reads also surfaced duplicate unresolved agent mappings on `APD-50557`/`APD-50759` and zero EDR matches on `APD-50712`/`APD-50213`, so client-wide EDR health is not overclaimed.
- **EDR/AV policy checkpoint (2026-09-19):** Owner confirmed every managed Windows endpoint must have Datto EDR and Datto AV unless explicit exception evidence exists. Deterministic evaluator added: zero Datto agent matches = confirmed gap; one current healthy authorized/licensed EDR+AV agent = good; healthy current + stale historical duplicate = needs cleanup; multiple current agents = gap; uniquely mapped Offline/Stale agent = unknown pending current evidence. Atomic `APD-50712` and `APD-50213` are therefore confirmed EDR/AV gaps; Defender does not satisfy this baseline.
- **Native monitoring checkpoint (2026-09-19):** Following the native-platform-first rule, Jason reused existing governed `endpoint.alert.search` rather than building a new alert collector. Exact endpoint-scoped reads covered all 25 Atomic Windows endpoints and returned 13 open alerts on 8 endpoints, with every returned alert independently bound to Atomic's exact site UID. This establishes a client-wide monitoring gap while leaving each alert to existing playbook/ticket/exception handling.
- **Remaining acceptance:** Convert the complete EDR/AV and monitoring evidence into the report surface using existing Jason evidence/reporting patterns; obtain/authorize IT Glue organization evidence without bypassing its information-release gate; preserve unavailable status for Endpoint Backup and Microsoft controls until their desk-dependent integrations are completed; add Grafana aggregate evidence only through the existing observability pattern.
- **Decision owner:** Jason Governance Authority / AOT Owner
...
- **Priority:** P2
- **Status:** Interim behavior defined — notification-only; broader workflow remains proposed
- **Risk level:** Moderate
...
- **Interim operating rule (2026-09-19):** When Jason discovers a new confirmed posture gap, notify the Owner with the affected client/control, concise evidence, affected devices when known, whether matching existing Autotask work was found, and a suggested next action. Do **not** create a new ticket, modify existing tickets, start remediation, or make configuration changes unless the Owner explicitly directs the next action.
...
### TODO-AI-005 — Production-compose shared AI security triage

- **Priority:** P1
- **Status:** In progress — structured evaluator, deterministic validation, registered prompt `PROMPT-SEC-001` v1.0.0, HOSTS-drift binding, production model composition, and unit acceptance are source-complete; live evidence/playbook acceptance remains
- **Risk level:** High
- **Idea:** Reuse one bounded AI security-triage service across security-oriented playbooks instead of embedding separate threat/benign prompts.
- **Implemented source:** `implementation/orchestrator/security_triage.py` plus `docs/architecture/Jason-AI-Security-Triage.md`. The evaluator has no tools or execution authority and can cite only supplied governed evidence.
- **Remaining work:** Emit PromptInvocation telemetry when TODO-AI-004 is implemented, pass evidence from a controlled real playbook investigation, and prove deterministic branching from assessment to playbook policy without allowing the model to authorize remediation or closure.
- **Acceptance:** One controlled security-oriented ticket supplies governed evidence to `PROMPT-SEC-001`; invented citations are impossible; classification and review requirement are persisted in the playbook run/audit evidence; policy chooses the next state; no action authority derives from the AI output; Grafana/Prompt Registry reflect the prompt/version and invocation when telemetry is available.
- **Decision owner:** Jason Governance Authority / AOT Owner
- **Review trigger:** Next security-playbook runtime composition session.
