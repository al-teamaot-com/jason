...
echo "========== START SEMANTIC EVIDENCE DIRECT RESOLUTION DEPLOYMENT AND PROBE =========="
...
mount_source() {
  local destination="$1"
  local inspect_json
  inspect_json="$(docker inspect jason-runtime)"
  INSPECT_JSON="$inspect_json" .venv/bin/python - "$destination" <<'PY'
import json
import os
import sys
...
Expected:
- Processor must resolve from provider-adapted semantic evidence and return the actual processor model.
- Windows Display Version must return a real Windows release only if Datto exposes authoritative mapped evidence; otherwise Jason must fail closed.
- Windows Display Version must never return health/version text.
EOF

echo "========== RESULT =========="
echo "Semantic evidence direct-resolution repair deployed and healthy."
echo "NO SOURCE CHANGES PERFORMED."
echo "========== END SEMANTIC EVIDENCE DIRECT RESOLUTION DEPLOYMENT AND PROBE =========="
