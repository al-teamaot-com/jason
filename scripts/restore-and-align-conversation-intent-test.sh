...
FILE="implementation/orchestrator/tests/test_conversation_resource_intent.py"
BASE_COMMIT="b9ded960ed77ed480b94745923429639a7c700a8"

echo "========== START CONVERSATION INTENT TEST RESTORE + ALIGN =========="

git show "${BASE_COMMIT}:${FILE}" > "${FILE}.restored"
python3 - <<'PY'
from pathlib import Path

src = Path("implementation/orchestrator/tests/test_conversation_resource_intent.py.restored")
dst = Path("implementation/orchestrator/tests/test_conversation_resource_intent.py")
...
'''
if old not in s:
    raise SystemExit("ERROR: expected baseline assertion not found")
s = s.replace(old, new, 1)
dst.write_text(s, encoding="utf-8")
src.unlink()
print("Restored full test file and aligned outcome assertion:", dst)
PY
...
echo "========== RESULT =========="
echo "Conversation intent tests restored and aligned."
echo "NO DEPLOYMENT PERFORMED."
echo "NO COMMIT OR PUSH PERFORMED."
echo "========== END CONVERSATION INTENT TEST RESTORE + ALIGN =========="
