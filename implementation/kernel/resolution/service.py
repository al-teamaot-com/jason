...
class GovernedCapabilityResolutionEngine:
    ...
    def __init__(self, *, capabilities: CapabilityRegistryService,
                 providers: ExecutionProviderRegistryService,
                 policy: ExecutionPolicyEngine,
                 translator: ProviderCandidateTranslator | None = None) -> None:
    ...
    def resolve(self, request: CapabilityResolutionRequest) -> CapabilityResolutionResult:
        ...
        if capability is None:
            return self._unresolved(request=request, status=status, reason_codes=("capability_not_found",))

        validation_result = self._validate_capability(request=request, capability=capability, status=status)
        if validation_result is not None:
            return validation_result

        context_result = self._validate_context(request=request, capability=capability, status=status)
        ...
        if not providers:
            return self._unresolved(request=request, capability=capability, status=status,
        ...
        return self._build_policy_result(request=request, capability=capability, status=status,
                                         providers=providers, decision=decision)

    def _resolve_capability(self, request: CapabilityResolutionRequest) -> tuple[CapabilityDefinition | None, CapabilityResolutionStatus]:
        try:
            if request.capability_version is not None:
                capability = self._capabilities.get(capability_name=request.capability_name,
                                                    version=request.capability_version)
                return capability, CapabilityResolutionStatus.RESOLVED_EXACT
            capability = self._capabilities.get_current(capability_name=request.capability_name,
    ...
    def _validate_capability(self, *, request: CapabilityResolutionRequest,
                             capability: CapabilityDefinition,
                             status: CapabilityResolutionStatus) -> CapabilityResolutionResult | None:
        ...
        if capability.lifecycle_status not in eligible_lifecycles:
            return self._unresolved(request=request, capability=capability,
    ...
    def _validate_context(self, *, request: CapabilityResolutionRequest,
                          capability: CapabilityDefinition,
                          status: CapabilityResolutionStatus) -> CapabilityResolutionResult | None:
        ...
        if capability.client_isolation_required and (request.client_id is None or not request.client_id.strip()):
            return self._unresolved(request=request, capability=capability,
                                    status=CapabilityResolutionStatus.ISOLATION_CONTEXT_MISSING,
                                    reason_codes=("client_context_required",))
        return None

    def _find_candidate_providers(self, *, request: CapabilityResolutionRequest,
                                  capability: CapabilityDefinition) -> tuple[ExecutionProvider, ...]:
    ...
    def _build_policy_result(self, *, request: CapabilityResolutionRequest,
                             capability: CapabilityDefinition,
                             status: CapabilityResolutionStatus,
                             providers: tuple[ExecutionProvider, ...],
                             decision: ExecutionDecision) -> CapabilityResolutionResult:
    ...
    @staticmethod
    def _unresolved(*, request: CapabilityResolutionRequest,
                    status: CapabilityResolutionStatus,
                    reason_codes: tuple[str, ...],
                    capability: CapabilityDefinition | None = None) -> CapabilityResolutionResult:
        return CapabilityResolutionResult(
            execution_id=request.execution_id,
            correlation_id=request.correlation_id,
            capability_name=request.capability_name,
            capability_version=capability.version if capability is not None else request.capability_version,
