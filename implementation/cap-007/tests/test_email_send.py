...
class FakeSecrets:
    ...
    def resolve(self, **kwargs):
        ...
        return SecretLease(values={"access_key_id": "AKIA_TEST", "secret_access_key": "secret-test"}, lease_id="lease-1")
...
def test_send_uses_logical_secret_and_returns_redacted_metadata():
    ...
    request = FakeRequest(arguments={"to": ["al@teamaot.com"], "subject": "CAP-007 test", "text_body": "Hello from Jason"})
    ...
    assert [event[0] for event in audit.events] == ["email.send.attempted", "email.send.completed"]
...
def test_audit_events_exclude_message_bodies_recipients_and_secret_values():
    ...
    request = FakeRequest(arguments={"to": [recipient], "subject": "Sensitive subject text", "text_body": body_secret, "html_body": f"<p>{body_secret}</p>"})
    service.invoke(request=request, resolution=FakeResolution())
    rendered = repr(audit.events)
    for forbidden in (body_secret, "secret-test", "AKIA_TEST", recipient, "Sensitive subject text"):
...
def test_provider_failure_emits_safe_failure_event_and_revokes_lease():
    service, secrets, transport, audit = invoker()
    transport.fail = True
    request = FakeRequest(arguments={"to": "al@teamaot.com", "subject": "provider failure", "text_body": "sensitive failure body"})
...
def test_provider_credentials_are_rejected_from_request_arguments():
    ...
    request = FakeRequest(arguments={"to": "al@teamaot.com", "subject": "bad", "text_body": "bad", "access_key_id": "should-not-be-here"})
...
def test_sender_must_be_policy_allowed():
    service, _, transport, audit = invoker()
    request = FakeRequest(arguments={"to": "al@teamaot.com", "subject": "bad sender", "text_body": "hello", "from_address": "someone@example.com"})
