...
def test_check_only_performs_no_retirement(monkeypatch, capsys, tmp_path: Path) -> None:
    monkeypatch.setattr(MODULE, "retire", lambda **kwargs: (_ for _ in ()).throw(AssertionError("retire must not be called")))
...
def test_parented_runtime_is_denied() -> None:
    original = MODULE.token_lookup_self
    MODULE.token_lookup_self = lambda address, token: {"parent": "parent-accessor"}
    try:
        try:
            MODULE.require_orphan_runtime("http://127.0.0.1:8200", "runtime")
        except RuntimeError as exc:
            assert "parented" in str(exc)
        else:
            raise AssertionError("Parented runtime token was not denied.")
    finally:
        MODULE.token_lookup_self = original


def test_successful_retirement_revalidates_after_revocation(monkeypatch, tmp_path: Path) -> None:
    bootstrap = protected(tmp_path / "bootstrap.token", "bootstrap")
    contract = protected(tmp_path / "contract.value", "contract")
    runtime = protected(tmp_path / "runtime.token", "runtime")
    ...
    monkeypatch.setattr(MODULE, "require_orphan_runtime", lambda address, token: calls.append("orphan"))
    monkeypatch.setattr(MODULE, "revoke_self", lambda address, token: calls.append("revoke"))
    ...
    assert calls == ["validate", "orphan", "revoke", "validate", "orphan"]
    ...
    assert stored["runtime_token_orphan"] is True
    assert stored["runtime_health_after_revocation"] == "approved"
...
def test_existing_evidence_blocks_before_runtime_or_revocation(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(MODULE.os, "geteuid", lambda: 0)
    monkeypatch.setattr(MODULE, "run_wrapper_validation", lambda **kwargs: calls.append("validate"))
    monkeypatch.setattr(MODULE, "require_orphan_runtime", lambda *args, **kwargs: calls.append("orphan"))
