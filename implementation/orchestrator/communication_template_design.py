...
class StructuredCommunicationClient(Protocol):
    def complete(self, *, system: str, user: str, schema: Mapping[str, Any], max_output_tokens: int = 1400) -> Mapping[str, Any]: ...
...
class CommunicationTemplateDesigner:
    ...
    def design(self, *, purpose_key: str, audience: str, scenario: str, approved_style_examples: Sequence[Mapping[str,Any]] = ()) -> CommunicationTemplateDesign:
        ...
        raw=self.client.complete(system=_SYSTEM_INSTRUCTIONS,user=json.dumps({"purpose_key":purpose_key.strip(),"audience":audience.strip(),"scenario":scenario.strip(),"approved_style_examples":[dict(x) for x in approved_style_examples]},ensure_ascii=False,separators=(",",":")),schema=_SCHEMA,max_output_tokens=1600)
