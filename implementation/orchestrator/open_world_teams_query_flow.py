...
from .open_world_discovery import OpenWorldResourceDiscovery
from .open_world_enrichment import OpenWorldCatalogEnricher
from .open_world_execution import OpenWorldExecutionCoordinator
from .open_world_query_planning import OpenWorldQueryPlanner
from .open_world_selector_grounding import OpenWorldGroundedSelectorResolver
from .open_world_semantic_mapping import OpenWorldSemanticMapper
from .teams_conversation_experience import (
    BoundTeamsConversationIntentExecutor,
    TeamsConversationExperienceResult,
)
from .teams_conversation_flow import TeamsConversationRequest
from .universal_teams_query_flow import render_governed_query_result


class OpenWorldUnsupportedInformation(RuntimeError):
    pass
...
class OpenWorldTeamsQueryFlow:
    ...
    def handle(
        ...
        try:
            # A correlation identifier grants no authority. Provider-global structural
            # metadata may be discovered under this trace before a target client is
            # known, while tenant data remains inaccessible until governed execution.
            correlation_id = self.fallback.request_factory.new_correlation_id()
            base_catalog = self.discovery.discover(correlation_id=correlation_id)

            stage = "classification"
            provisional = unknown_attribution(
                organization_id=(
                    os.getenv("JASON_ORGANIZATION_ID", "aot").strip() or "aot"
                ),
            ...
            with bind_attribution_context(provisional):
                classification = self.classifier.map(
                    human_text=question,
            ...
            if not classification.information_request:
                print(
                    "JASON_OPEN_WORLD_BYPASS reason=non_information",
                    file=sys.stderr,
                    flush=True,
                )
                return self.fallback.handle(request)

            # Once classified as operational information, the request stays on the
            # governed information path. Missing fields or downstream failures never
            # fall back into the old closed-world kernel.
            stage = "identity_binding"
            principal = self.fallback.identity_binder.bind(request.identity)
            ...
            executor = BoundTeamsConversationIntentExecutor(
                request_factory=self.fallback.request_factory,
                orchestrator=self.fallback.orchestrator,
                principal=principal,
            ...
            with bind_attribution_context(attribution):
                ...
                grounding = self.selector_grounder.ground(
                    human_text=question,
                    context=context,
                    catalog=base_catalog,
                )
                ...
                catalog = self.enricher.enrich(
                    ...
                    executor=executor,
                    selector_references=dict(grounding.enrichment_selectors),
                )
                ...
                plan = self.selector_grounder.bind_plan(
                    plan=plan,
                    catalog=catalog,
                    selectors_by_handle=grounding.selectors_by_handle,
                )
                ...
                response_text = render_governed_query_result(
                    result=execution.query_result,
                    answer_mode=plan.answer_mode,
                )
                orchestrations = tuple(execution.orchestrations)

            print(
                "JASON_OPEN_WORLD_SUCCESS"
                + " correlation_id=" + str(correlation_id)
                + " sources=" + str(len(plan.sources))
                + " joins=" + str(len(plan.relational_plan.joins))
                + " rows=" + str(len(execution.query_result.rows))
                + " columns=" + str(len(execution.query_result.columns))
                + " answer_mode=" + str(plan.answer_mode),
                file=sys.stderr,
                flush=True,
            )

        except Exception as error:
            error_correlation = correlation_id or "unassigned"
            print(
            ...
            orchestrations = (
                tuple(executor.results) if executor is not None else tuple()
            )
            if correlation_id is None:
                correlation_id = self.fallback.request_factory.new_correlation_id()

        message_id = self.fallback.transport.send(
