...
def test_verified_provider_relationship_can_be_promoted() -> None:
    ...
    evidence = ProviderRelationshipEvidence(
        provider="datto_rmm",
        source=ref("datto_rmm", "device", "device-1"),
        target=ref("autotask", "company", "208"),
        provider_relationship="site_company_mapping",
        canonical_relationship="belongs_to",
        verification=VerificationState.VERIFIED,
        confidence=1.0,
        observed_at=observed,
        source_authority="governed-provider-read",
    )
...
def test_unverified_provider_link_cannot_become_canonical_truth() -> None:
    evidence = ProviderRelationshipEvidence(
        provider="it_glue",
        source=ref("it_glue", "configuration", "10"),
        target=ref("datto_rmm", "device", "device-1"),
        provider_relationship="name_match",
        canonical_relationship="represents",
        verification=VerificationState.INFERRED,
        confidence=0.6,
        observed_at=datetime.now(timezone.utc),
        source_authority="bounded-discovery",
    )

    with pytest.raises(ValueError, match="corroborated or verified"):
        promote_provider_evidence(
            evidence,
...
def test_cross_organization_provider_link_fails_closed() -> None:
    with pytest.raises(ValueError, match="Cross-organization"):
        ProviderRelationshipEvidence(
            ...
            provider_relationship="email_match",
            canonical_relationship="represents",
...
def test_unknown_relationship_type_fails_closed() -> None:
    with pytest.raises(ValueError, match="Unknown canonical relationship"):
        ProviderRelationshipEvidence(
            ...
            source=ref("it_glue", "configuration", "10"),
            target=ref("autotask", "company", "208"),
            provider_relationship="custom",
            canonical_relationship="looks_like",
            verification=VerificationState.VERIFIED,
            confidence=1.0,
            observed_at=datetime.now(timezone.utc),
            source_authority="governed-provider-read",
