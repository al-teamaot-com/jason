...
def test_plan_uses_generic_resource_gateway_and_existing_connectors():
    plan = build_configuration_device_plan(
        organization_id="org-208",
        configuration_id="42",
        search_hint="OWNi7PC1223B",
    )

    assert plan.reads[0].query.provider == "it_glue"
    assert plan.reads[0].invocation.capability == "it_glue.entity.get"
    assert plan.reads[0].invocation.arguments == {
        "entity": "Configurations",
        "entity_id": "42",
    }
    assert plan.reads[1].query.provider == "datto_rmm"
    assert plan.reads[1].invocation.capability == "datto_rmm.device.search"
    assert plan.reads[1].invocation.arguments == {"search": "OWNi7PC1223B"}


def test_executor_preserves_active_organization_context():
    it_glue = RecordingConnector(
        "it_glue",
        frozenset({"it_glue.entity.get"}),
    )
    datto = RecordingConnector(
        "datto_rmm",
        frozenset({"datto_rmm.device.search"}),
    )
