...
from autonomous_remediation.datto_edr_av_playbook import PLAYBOOK_VERSION
        ...
        with self._connection:
            self._connection.execute(
                ...
                INSERT INTO autonomy_operational_work(
                    ticket_id,ticket_number,title,source_queue,company_id,
...
class GovernedAutonomyActionPort:
    ...
    def _standing_policy(self, capability: str) -> StandingPolicyAuthorization:
        promotion = self.promotion_store.find_scope_approved(
            playbook_id=PLAYBOOK_ID,
            playbook_version=PLAYBOOK_VERSION,
            policy_id=POLICY_ID,
            required_capabilities=REQUIRED_ACTION_CAPABILITIES,
        )
        if promotion is None or capability not in promotion.allowed_capabilities:
            raise OperationalAutonomyError(
                "EDR/AV autonomous playbook lacks exact durable owner promotion"
            )
    ...
    def execute(self, capability: str, arguments: Mapping[str, Any]) -> Mapping[str, Any]:
        request = self.request_factory.build(
            ...
            client_id=None,
            standing_policy=self._standing_policy(capability),
        )
...
class OperationalAutonomyMaintenance:
    ...
    def tick(self) -> None:
        ...
        eligible = [
            ...
            if int(item.resource_id) not in processed
            and self.store.get(int(item.resource_id)) is None
            and self._is_health_only_edr_ticket(item.context)
        ]
        eligible.sort(
            key=lambda item: (
                -int(item.urgent),
        ...
        for candidate in eligible[:slots]:
            try:
                work = self._admit(candidate)
                self._advance(work, candidate.context)
            except Exception as exc:
                self._record_admission_failure(candidate, exc)
    ...
    def _admit(self, candidate) -> OperationalWork:
    ...
    def _advance(self, work: OperationalWork, ticket: Mapping[str, Any]) -> None:
        if work.phase == "claim":
            ...
            work = self._replace(work, phase="health_dispatch")
            self.store.put(work)
    ...
    def _dispatch_component(
        ...
        output = self.actions.execute(
            "automation.component.execute",
            {
                ...
                "variables": {},
                "job_name": f"Jason autonomous {PLAYBOOK_ID} {step} T{work.ticket_id}",
                "idempotency_key": (
                    f"autonomy:{PLAYBOOK_ID}:{PLAYBOOK_VERSION}:"
        ...
        self.store.put(
            self._replace(
                ...
                last_reason=f"Dispatched {identity.name}.",
            )
        )
    ...
    def _complete(self, work: OperationalWork, stdout: str) -> None:
        ...
        self._write_note(work.ticket_id, note, "Jason - Autonomous EDR/AV Resolution")
        self.actions.execute(
            "service.ticket.update",
    ...
    def _escalate(self, work: OperationalWork, reason: str) -> None:
        self._write_note(
            ...
            "Jason - Autonomous EDR/AV Escalation",
        )
        self.store.put(
            self._replace(
                work,
    ...
    def _record_admission_failure(self, candidate, error: Exception) -> None:
    ...
    def _write_note(self, ticket_id: int, body: str, title: str) -> None:
        self.actions.execute(
            "service.ticket.note.create",
            {
                "payload": {
