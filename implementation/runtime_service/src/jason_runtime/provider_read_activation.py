...
PROVIDER_READ_ENTRA_PROCUREMENT_MAIL_CATALOG_PROFILE = (
    "itglue-autotask-entra-procurement-mail-catalog-v6"
)
PROVIDER_READ_ENTRA_PROCUREMENT_MAIL_CONTRACT_CATALOG_PROFILE = (
    "itglue-autotask-entra-procurement-mail-contract-catalog-v7"
)
...
_V4_EXPECTED_PROVIDER_CATALOGS = {
    **_V3_EXPECTED_PROVIDER_CATALOGS,
    MICROSOFT_GRAPH_PROVIDER: MICROSOFT_GRAPH_DIRECTORY_CAPABILITIES,
}

_AUTOTASK_PRE_CONTRACT_CAPABILITIES = AUTOTASK_CAPABILITIES - frozenset({
    SERVICE_CONTRACT_SEARCH, SERVICE_CONTRACT_READ
})

_V5_EXPECTED_PROVIDER_CATALOGS = {
    IT_GLUE_PROVIDER: IT_GLUE_CAPABILITIES,
    AUTOTASK_PROVIDER: _AUTOTASK_PRE_CONTRACT_CAPABILITIES,
    MICROSOFT_GRAPH_PROVIDER: MICROSOFT_GRAPH_DIRECTORY_CAPABILITIES,
}

_V6_EXPECTED_PROVIDER_CATALOGS = {
    IT_GLUE_PROVIDER: IT_GLUE_CAPABILITIES,
    AUTOTASK_PROVIDER: _AUTOTASK_PRE_CONTRACT_CAPABILITIES,
    MICROSOFT_GRAPH_PROVIDER: MICROSOFT_GRAPH_CAPABILITIES,
}
_AUTOTASK_PRE_RESOURCE_CAPABILITIES = AUTOTASK_CAPABILITIES - frozenset({
    SERVICE_RESOURCE_SEARCH, SERVICE_RESOURCE_READ,
})
_AUTOTASK_PRE_ATTACHMENT_CAPABILITIES = _AUTOTASK_PRE_RESOURCE_CAPABILITIES - frozenset({
    SERVICE_TICKET_ATTACHMENT_SEARCH, SERVICE_TICKET_ATTACHMENT_READ,
    SERVICE_TICKET_ATTACHMENT_CONTENT_READ,
})
_V7_EXPECTED_PROVIDER_CATALOGS = {
    IT_GLUE_PROVIDER: IT_GLUE_CAPABILITIES,
    AUTOTASK_PROVIDER: _AUTOTASK_PRE_ATTACHMENT_CAPABILITIES,
    MICROSOFT_GRAPH_PROVIDER: MICROSOFT_GRAPH_CAPABILITIES,
}
_V8_EXPECTED_PROVIDER_CATALOGS = {
    IT_GLUE_PROVIDER: IT_GLUE_CAPABILITIES,
    AUTOTASK_PROVIDER: _AUTOTASK_PRE_RESOURCE_CAPABILITIES,
    MICROSOFT_GRAPH_PROVIDER: MICROSOFT_GRAPH_CAPABILITIES,
}
...
PROVIDER_READ_ENTRA_PROCUREMENT_MAIL_CATALOG_CAPABILITIES = frozenset().union(
    *(
        _V6_EXPECTED_PROVIDER_CATALOGS[provider_id]
        for provider_id in sorted(_V6_EXPECTED_PROVIDER_CATALOGS)
    )
)
PROVIDER_READ_ENTRA_PROCUREMENT_MAIL_CONTRACT_CATALOG_CAPABILITIES = frozenset().union(
    *(
        _V7_EXPECTED_PROVIDER_CATALOGS[provider_id]
        for provider_id in sorted(_V7_EXPECTED_PROVIDER_CATALOGS)
    )
)
PROVIDER_READ_ENTRA_PROCUREMENT_MAIL_CONTRACT_ATTACHMENT_CATALOG_CAPABILITIES = frozenset().union(
    *(
        _V8_EXPECTED_PROVIDER_CATALOGS[provider_id]
        for provider_id in sorted(_V8_EXPECTED_PROVIDER_CATALOGS)
    )
)
PROVIDER_READ_ENTRA_PROCUREMENT_MAIL_CONTRACT_ATTACHMENT_RESOURCE_CATALOG_CAPABILITIES = frozenset().union(
    *(
...
def apply_provider_read_activation_profile(
    *,
    ...
    elif normalized == PROVIDER_READ_ENTRA_PROCUREMENT_MAIL_CATALOG_PROFILE:
        expected_catalogs = _V6_EXPECTED_PROVIDER_CATALOGS
        approved_capabilities, provider_capabilities = _governed_catalog_contract(
            expected_catalogs
        )
    elif normalized == PROVIDER_READ_ENTRA_PROCUREMENT_MAIL_CONTRACT_CATALOG_PROFILE:
        expected_catalogs = _V7_EXPECTED_PROVIDER_CATALOGS
        approved_capabilities, provider_capabilities = _governed_catalog_contract(
            expected_catalogs
        )
    elif normalized == PROVIDER_READ_ENTRA_PROCUREMENT_MAIL_CONTRACT_ATTACHMENT_CATALOG_PROFILE:
        expected_catalogs = _V8_EXPECTED_PROVIDER_CATALOGS
        approved_capabilities, provider_capabilities = _governed_catalog_contract(
            expected_catalogs
        )
